...
- v1.9 — US-214: Volumetric Nebula Rendering — `buildLayers(emotionWeights)` produces up to 7 sphere layers per zone: 2 per qualifying emotion (outer diffuse + inner concentration glow, max 3 emotions, threshold `w > 0.05`) + 1 ambient tie layer (`#78716C`); each layer animated independently via `useFrame` sin/cos on separate scale axes (`axisScale`); `ACTIVE_OPACITY_MULTIPLIER=1.6` boosts all layer opacities on hover; `layerRefs = useRef<(Mesh | null)[]>([])` per-layer ref array; `StarField.tsx` replaced single dominant-color prop with proportional `emotionWeights: Record<string, number>` (count/total per emotion); forbidden: dominant single-color fog when multiple emotional tones present
- v1.8 — US-208: Life Area Zones — Emergent Clusters — `aiService.classifyLifeArea(storyContent, existingAreas, apiKey)` (returns emergent label or null); `LIFE_AREA_SYSTEM_PROMPT` (fully emergent, forbidden preset labels, prefers matching existing areas); `updateStoryLifeArea()` in storyService; `useLifeAreaZones.ts` hook (localStorage key `dotflow_zone_labels`, rename + clear labels); `LifeAreaZone.tsx` component (sphere nebula overlay, invisible hit mesh, depth-priority hover system); `getZoneLocalPosition()` + `getFixedZoneCentroid()` in starPositions.ts (stable, no near-side bias); depth-priority zone selection via `zoneDistancesRef` Map + `activateNearestZone()` (nearest zone wins, stars unblocked); hard containment invariant (Pass 3b clamps zone stars to radius − 0.2); drag threshold on StarNode/StoryNode/BlackHole (5px, `DRAG_THRESHOLD_SQ=25`); backfill life_area for existing stories on sky open; **🎉 All M3 P0/P1 gate blockers resolved**
- v1.7 — InsightModal click behavior + SkyModal: `SkyModal` reusable backdrop modal (X button aria-label "Zamknij", Escape key, backdrop click, optional `footer` prop); `InsightModal` opened by clicking black hole — button flow: initial state "Jest OK" + "Rozwiń" → after "Rozwiń": elaboration loaded + note textarea + "To ma sens" (left) + "Zapisz →" (right, disabled when empty); acknowledgment persisted in `dotflow_acknowledged_insight` (keyed by insight text), elaboration cached in `dotflow_elaborated_insight`, note saved to `dotflow_insight_note` (current) and `dotflow_insight_notes` (history, last 50); `generateHolisticInsight(entries, apiKey, previousNote?)` incorporates saved note into next insight prompt; `/flow` skill removed — qa now goes directly to /docs
